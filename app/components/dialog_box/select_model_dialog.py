...
class SelectModelDialog(MaskDialogBase):
    ...
    def __init__(self, selectedModel: str, parent):
        ...
        self.titleLabel = QLabel(
            self.tr('Select Face Mask Detector from local model library'), self.widget)
    ...
    def __initLayout(self):
        """ 初始化布局 """
        w = max(325, self.contentLabel.width()+48, self.titleLabel.width()+48)
        self.widget.setFixedSize(w, 324)

        self.titleLabel.move(25, 25)
        self.contentLabel.move(25, 63)
    ...
    def showDeleteModelPanel(self):
        ...
        w.yesSignal.connect(self.deleteModelFolder)
        w.exec_()
    ...
    def showFileDialog(self):
        ...
        path, _ = QFileDialog.getOpenFileName(self, self.tr("Select model"), "./model", self.tr(
            'Model file') + ' (*.pth; *.pt);;' + self.tr('All files') + ' (*.*)')
    ...
    def __adjustWidgetsPos(self):
        """ 调整卡片位置 """
        w = max(352, self.contentLabel.width() +
                48, self.titleLabel.width()+48)
        self.widget.setFixedSize(w, 259)
        self.completeButton.move(
            w-24-self.completeButton.width(), self.widget.height()-57)

        self.addModelCard.move((w-292)//2, 104)
        if self.modelCard:
            self.modelCard.move((w-292)//2, 104)
            self.addModelCard.hide()
...
class ModelCard(FoldingWindow):
    ...
    def paintEvent(self, e):
        ...
        if self.pressedPos in ["left-top", "top", "right-bottom"]:
            ...
            self.paintText(painter, 11, 8, 12, 7)
        elif self.pressedPos in ["left", "center", "right"]:
            self.paintText(painter, 12, 8, 12, 7)
        elif self.pressedPos in ["left-bottom", "bottom", "right-top"]:
            painter.shear(0.05, 0)
            self.paintText(painter, 10, 8, 10, 7)
        else:
            self.paintText(painter, 10, 9, 10, 10)
        # 绘制叉号
